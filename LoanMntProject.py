...
class Customer:
    def __init__(self,root):
        ...
        title=Label(self.root,text="Loan Management System",font=("times new romman",20,'bold'),bd=10,relief=GROOVE,bg='Blue',fg='red')
...
#=============================Functions================================
    ...
    def addrecord(self):
        if self.loalId.get() == '' or self.name.get() == '' or self.mob.get() == '' or self.aadhar.get() == '' or self.add.get() == '' or self.pin.get()=='':
            messagebox.showerror('Error', 'Employee Details are must ?')
        else:
            self.total()
            con = mysql.connector.connect(host="localhost", user="root", password="", database="dmp")
            cur = con.cursor()
    ...
    def update(self):
        if self.loalId.get()=='':
            ...
            con = mysql.connector.connect(host="localhost", user="root", password="Lokesh@8788", database="dmp")
            cur = con.cursor()
            sql = f"UPDATE employee WHERE loalId = self.loalId.get()"
            
            #sql= f"UPDATE employee SET name=%s,year=%s,rate=%s,mpay=%s,tpay=%s,mob= %s ,aadhar=%s,add=%s,pin=%s,amount=%s, {(self.name.get(),self.year.get(),self.rate.get(),self.mpay.get(),self.tpay.get(),self.mob.get(),self.aadhar.get(),self.add.get(), self.pin.get(),self.amount.get())}  WHERE loanId = {self.loalId.get()}"
            #sql= "UPDATE employee SET name= 'lokesh k'  WHERE loanId ='987'"
    ...
    def delete(self):
        # if  self.loalId=loalId.get()
        if self.loalId.get()=='':
            messagebox.showerror('Error','Please enter Loan ID to delete the records')
        else:
            con = mysql.connector.connect(host="localhost", user="root", password="Lokesh@8788", database="dmp")
            cur = con.cursor()
            print(self.loalId.get())
            print("%s")
            
            sql = f"DELETE FROM employee WHERE loanId = {self.loalId.get()}"
            # sql="delete from employee where loanId=%self"
            # val=(self.loalId)
            # cur.execute(sql,val)
            ...
            # cur.execute("delete  from employee where loanId= '%s'")
            con.commit()
            con.close()
            self.fatch_data()
            self.reset()
